...
(function($) {
	...
	RSS.prototype.render = function() {
		...
		this.load(function(data) {
			...
			if (self.effectQueue.length > 0) {
				...
				$.isFunction(self.callback) && self.callback.call(this, self.target);
			}
		})
	}
	...
	RSS.prototype.generateHTMLForEntries = function() {
		...
		$(this.entries).each(function() {
			...
			if (self.isRelevant(entry, result.entries.length)) { //wesley
	...
	RSS.prototype.isRelevant = function(entry, entryCount) { //wesley
		var tokenMap = this.getTokenMap(entry)

		if (this.options.filter) {
			if (this.options.filterLimit && (this.options.filterLimit == entryCount)) {
				return false
			} else {
